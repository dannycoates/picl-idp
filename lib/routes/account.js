  ...
  var routes = [
      ...
      config: {
        validate: {
          payload: {
            ...
            device: isA.object({
              id: isA.string().length(4).regex(HEX_STRING).optional(),
              name: isA.string().max(64).optional(),
              type: isA.string().optional(),
              callbackURL: isA.string().optional()
            }).optional()
          }
        }
      },
                  ...
                  .then(
                    function (account) {
                      ...
                      .then(
                        function (response) {
                          if (form.device) {
                            form.device.uid = account.uid
                            form.device.sessionTokenId = response.sessionToken.tokenId
                            return db.upsertDevice(form.device)
                              .then(
                                function (device) {
      ...
      config: {
        ...
        response: {
          schema: {
            ...
            device: isA.object({
              id: isA.string().length(4).regex(HEX_STRING).required(),
              uid: isA.string().regex(HEX_STRING).required(),
          ...
          .then(
            function (emailRecord) {
              ...
              .then(
                function (tokens) {
                  if (form.device) {
                    form.device.uid = emailRecord.uid
                    form.device.sessionTokenId = tokens.sessionToken.tokenId
                    return db.upsertDevice(form.device)
                      .then(
                        function (device) {
                          tokens.device = butil.unbuffer(device)
                          return tokens
                        },
                        function (err) {
    ...
    },
    {
      ...
      handler: function (request, reply) {
        ...
        payload.uid = sessionToken.uid
        if (!payload.id) {
          // a fresh device gets bound to the current session
          payload.sessionTokenId = sessionToken.tokenId
        }
        db.upsertDevice(payload).then(
          function (device) {
