...
module.exports = function (
  ...
  DB.prototype.upsertDevice = function (deviceInfo) {
    log.trace({ op: 'DB.upsertDevice', uid: deviceInfo.uid, id: deviceInfo.id })
    if (!deviceInfo.id) {
      deviceInfo.id = crypto.randomBytes(16)
      deviceInfo.createdAt = Date.now()
    }
    return this.pool.put(
      '/account/' + deviceInfo.uid.toString('hex') +
